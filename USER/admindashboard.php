  ...
  <div class="main">
    ...
    <!-- Stats -->
        <?php
          ...
          $total_sql = "SELECT COUNT(*) AS total FROM booking_data";
          $total_result = $conn->query($total_sql);
          $total_requests = $total_result->fetch_assoc()['total'];

          // Approved Requests
          $approved_sql = "SELECT COUNT(*) AS approved FROM booking_data WHERE status='approved'";
          $approved_result = $conn->query($approved_sql);
          $approved_requests = $approved_result->fetch_assoc()['approved'];

          // Pending Requests
          $pending_sql = "SELECT COUNT(*) AS pending FROM booking_data WHERE status='pending'";
          $pending_result = $conn->query($pending_sql);
          $pending_requests = $pending_result->fetch_assoc()['pending'];

          // Rejected Requests
          $rejected_sql = "SELECT COUNT(*) AS rejected FROM booking_data WHERE status='rejected'";
          $rejected_result = $conn->query($rejected_sql);
          $rejected_requests = $rejected_result->fetch_assoc()['rejected'];
    ...
    <div class="stats">
      ...
      <div class="card"><h3>Approved</h3><p><?php echo $approved_requests; ?></p></div>
      <div class="card"><h3>Pending</h3><p><?php echo $pending_requests; ?></p></div>
      <div class="card"><h3>Rejected</h3><p><?php echo $rejected_requests; ?></p></div>
    </div>
    ...
    <div class="table-container">
      ...
      <table>
        ...
        <tbody>
          ...
          <?php
          include 'connection.php';
      $sql = "SELECT * FROM booking_data where status = 'pending' " ;
